...
window.addEventListener("load", async () => {
  ...
  const data = JSON.parse(await (await fetch(new Request('./static/locations.json'))).text());
	...
	loader.load('static/arrow.gltf', function (gltf) {
		...
		const yellowLightFront = new THREE.PointLight(0x876800, 4);
		yellowLightFront.position.set(0, 2.75, 0);
		arrowGroup.add(yellowLightFront);

		const yellowLightTop = new THREE.PointLight(0x876800, 4);
		yellowLightTop.position.set(0, 2.5, 0.2);
		arrowGroup.add(yellowLightTop);

		const yellowLightBottom = new THREE.PointLight(0x876800, 4);
		yellowLightBottom.position.set(0, 2.5, -0.2);
		arrowGroup.add(yellowLightBottom);

		// Add a light to the group
		const light = new THREE.PointLight(0x808080, 1000);
